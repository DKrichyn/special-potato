...
export const App = () => {
  ...
  const updateVector = (index: number, value: number) => {
    const nextVector = input.vector.map((item, itemIndex) => (itemIndex === index ? value : item));
    solver.setInput({ ...input, vector: nextVector });
  };
    ...
    <div className={styles.app}>
      ...
      {inputMode === 'file' ? (
        ...
        <div className={styles.grid}>
          ...
          <div className={styles.card}>
            <h3 className={styles.sectionTitle}>Vector B</h3>
            <div className={styles.vectorInputs}>
              {Array.from({ length: size }).map((_, index) => (
                <div key={index} className={styles.inlineField}>
                  <label htmlFor={`b-${index}`}>b{index + 1}</label>
                  <input
                    id={`b-${index}`}
                    className={styles.smallInput}
                    type="number"
                    value={input.vector[index] ?? 0}
                    onChange={(event) => updateVector(index, Number(event.target.value))}
                  />
                </div>
              ))}
            </div>
