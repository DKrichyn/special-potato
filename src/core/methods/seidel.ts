...
import { IterationEntry, IterativeParams, MethodResult, Matrix, Vector } from '../types';

const ZERO_DIAGONAL_TOLERANCE = 1e-12;

/**
 * Solve a system of linear equations using the Gauss–Seidel iterative method.
 * Validates input dimensions, performs iterative updates, and stops when the
 * difference norm between successive approximations drops below epsilon or
 * when maxIterations is reached.
 */
export function solveSeidel(A: Matrix, B: Vector, params: IterativeParams): MethodResult {
  const size = A.length;

  if (size === 0) {
    return { method: 'seidel', x: null, iterationLog: [], iterations: 0, converged: false,
      isConverged: false, errorMessage: 'Matrix must not be empty.' };
  }
  ...
  for (let iteration = 1; iteration <= params.maxIterations; iteration += 1) {
    ...
    for (let i = 0; i < size; i += 1) {
      ...
      for (let j = 0; j < size; j += 1) {
        if (j === i) continue;
        sum += A[i][j] * (j < i ? next[j] : current[j]);
      }
